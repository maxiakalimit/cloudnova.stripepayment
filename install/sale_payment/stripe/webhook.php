...
try {
    ...
    if ($orderId) {
        ...
        $events = GetModuleEvents("cloudnova.stripepayment", "OnBeforeUpdateOrder", true);
        ...
        $events = GetModuleEvents("cloudnova.stripepayment", "OnBeforeSuccessOutput", true);
        foreach ($events as $arEvent) {
            ExecuteModuleEventEx($arEvent, array(&$output,$arFields,$orderID));
        }

        if(!empty($SALE_CORRESPONDENCE['REDIRECT_SUCCESS']['VALUE'])){
            $url = CloudnovaModuleStripeClass::GetRedirectUrl($SALE_CORRESPONDENCE['REDIRECT_SUCCESS']['VALUE'],$orderId,'success');
            if($url) {
                LocalRedirect($url);
            }
        }
