...
\Bitrix\Main\Loader::registerAutoLoadClasses(
    "cloudnova.stripepayment",
    array(
        'cloudnova\StripeHelper' => 'sale_payment/stripe/lib/StripeHelper.php',
    )
);

\Bitrix\Main\Loader::includeModule('cloudnova.stripepayment');
...
$sum = $arOrder['PRICE'];
$sum = \number_format($sum, 2, '.', '');
if(!CloudnovaModuleStripeClass::isSupportCurrency($arOrder['CURRENCY'])) {
...
$events = GetModuleEvents("cloudnova.stripepayment", "OnBeforeShowStripe", true);
...
//            'currency' => $arOrder['CURRENCY'],
    ...
    $events = GetModuleEvents("cloudnova.stripepayment", "OnBeforeStripeCreateSession", true);
    foreach ($events as $arEvent) {
...
//        die();
    ...
    // Возвращаем ответ в формате JSON если редиректа не было
...
$fileTemplate = CloudnovaModuleStripeClass::GetPathTemplate($SALE_CORRESPONDENCE['STRIPE_TEMPLATE']['VALUE']);
